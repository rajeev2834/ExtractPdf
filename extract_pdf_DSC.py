...
def process_files(batch):
    search_keywords = ['Clust er ID', 'A C No.', 'P art No.', 'Epic No.', 'Serial No. In P art', 'Name', "Relative's Name"]
    batch_results = []
            ...
            try:
                with open(filename, 'rb') as pdfFileObject:
                    ...
                    for keyword in search_keywords:
                        if keyword == 'Name':
                            pattern = r'^(?!.*District\s+Name).*\bName:\s*(.*)$'
                        else:
...
def main(argv):
    path = 'C:/Users/WIN 10/Desktop/New folder/AC 239 MISSING DSE 27.12.2023'
    ...
    df.rename(columns={
        ...
        'P art No.': 'Part_No',
        'Epic No.': 'Epic No',
        'Serial No. In P art': 'Sl.No_Part',
        "Relative's Name": 'Rel.Name',
    }, inplace=True)
    ...
    df.to_csv('C:/Users/WIN 10/Desktop/ExtractPdf/DSE_239.csv', index=False)  # Saving as CSV for large data
    return df
